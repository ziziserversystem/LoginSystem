...
class LoginSystem extends PluginBase implements Listener {
    ...
    function onEnable() {
        $plugin = "LoginSystem";
        $this->lasttime = new Config($this->getDataFolder()."lasttime.yml", Config::YAML);
    ...
    function onLogin(PlayerPreLoginEvent $event) {
        ...
        if ($this->isClientBan($cid)) {
            ...
            if ($r["ip"] != $ip && $r["cid"] != $cid) {
                $event->setCancelled();
                $event->setKickMessage("§a【運営】 §cログインデータが一致しません");
            }
        } elseif (!empty($R)) {
            if ($r["name"] != $name) {
                $event->setCancelled();
    ...
    function onJoin(PlayerJoinEvent $event) {
        ...
        $this->lasttime->set($name,$time);
        $this->lasttime->save();
        if (empty($r)) {
            $player->sendMessage("§e【運営】 §fこのサーバーではログイン認証を行っています\n§e【運営】 §fアカウント登録画面が出ない場合 /register と打ってください");
            ...
            if ($r["ip"] == $ip && $r["cid"] == $cid) {
                $player->sendMessage("§e【運営】 §fログイン認証がされました");
            } else {
            	$this->log[$name] = 0;
                $player->sendMessage("§e【運営】 §cログインデータが一致しませんでした\n§e【運営】 §fパスワードでログインできます\n§e【運営】 §cログイン画面が出ない場合 /login と打ってください");
                $player->setImmobile(true);
        		...
        		if (empty($r)) {
            		if (preg_match("/[^a-zA-Z0-9]/", $data[1]) or $data[1] === "") {
            			...
            			if (isset($data[1]) and ($data[1] === $data[2])) {
                    		...
                    		$player->sendMessage("§e【運営】 §fアカウント登録をしました\n§e【運営】 §fパスワードは< " . $data[1] . " >です\n§e【運営】 §cスクリーンショットをして保存してください");
            		...
            		if ($r["data"] == "2") {
                		...
                		if ($r["pass"] == $p) {
                    		...
                    		$player->sendMessage("§e【運営】 §fパスワードで認証されました");
	                    	$this->log[$name] = 0;
	                    	$player->setImmobile(false);
                		} else {
	                    	$this->log[$name]++;
    ...
    function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
        switch ($command->getName()) {
        		...
        		if ($sender instanceof Player) {
        			...
        			if (empty($r)) {
        				...
        				if ($r["data"] == "2") {
        					...
        					$player->sendMessage("§e【運営】 §fログイン認証がされています");
        				}
        			}
                }
                break;
            ...
            case "lasttime":
                if(!isset($args[0])){
                    $sender->sendMessage("§a【運営】 §eプレイヤーの名前を入力してください");
			 return true;
		 }

		   if(!$this->lasttime->exists($args[0])){
			 $sender->sendMessage("§a【運営】 §eそのプレイヤーは存在しません");
			  return true;
		 }

	  	 $time = $this->lasttime->get($args[0]);
		 $sender->sendMessage("§a【運営】 §f{$args[0]} §eの最終ログイン日時は§b {$time} §eです");
